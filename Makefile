...
CONTAINER_NAME=ecommerce-white-label-backend
...
.PHONY: docker/build
docker/build:
	docker build -t ecommerce-white-label-backend:latest -f .setup/build/Dockerfile --build-arg GITLAB_USER=${GITLAB_USER} --build-arg GITLAB_PASSWORD=${GITLAB_PASSWORD} .
...
.PHONY: docker/bash
docker/bash:
	docker exec -it ecommerce-white-label-backend sh

## docker/logs: start the local log
.PHONY: docker/logs
docker/logs:
	docker logs --tail 1000 -f ecommerce-white-label-backend
